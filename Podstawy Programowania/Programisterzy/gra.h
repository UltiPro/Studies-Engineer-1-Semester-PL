...
void graj()
{
    ...
    {
        czysc();
        printf("Nie udalo sie otworzyc listy pytan do odczytu! Powrot do menu!");
        sleep(3);
        czysc();
        main();
    }
    ...
    for (int i = 0; i < ile_pytan; i++) // petla wyswietla kolejne pytania, zmienna i oznacza numer pytania
    {
        ...
        fscanf(plik, "%[^\n]", pytanie);
        przesun_o_jedna_linie(znak, plik); // pobiera z pliku tresc pytanie,mozliwych opd i [poprawnej
        fscanf(plik, "%[^\n]", odpA);
        ...
        printf("Twoj wybor: ");
        znak = getchar();
        getchar();                                                                                         // pobiera odpowiedz uzytkownika
