int piedziesiat_na_piedziesiat(int nr, char pytanie[100], char odpA[100], char odpB[100], char odpC[100], char odpD[100], char popodp[2], char znak)
{
    ...
    pasek();
    printf("%s\n", pytanie);
    int nr1 = 0, nr2 = 0;  // s� to odpowiedzi kt�re odrzucimy w kole 50 na 50
    nr1 = rand() % 4 + 65; // losujemy numer1 w zakresie 65-68 odpowiadaj�ce w kodzie ASCI A-D
    while (nr1 == popodp[0])
        nr1 = rand() % 4 + 65; // losujemy dop�ki numer1 pokrywa si� z poprawn� odpowiedzi�
    nr2 = rand() % 4 + 65;
    while (nr2 == popodp[0] || nr2 == nr1)
        nr2 = rand() % 4 + 65; // losujemy nr2 tak aby nie pokty� si� z odpowiedzi� poprawn� ani z nr1
    if (popodp[0] == 65 || (nr1 != 65 && nr2 != 65))
        printf("A. %s\n", odpA); // na ekram wypisujemy te odpowiedzi kt�re: s� odpowiedzi� poprawn� lub nie si� nr1 i nr2 - czyli odpowiedziami wykluczonymi
    if (popodp[0] == 66 || (nr1 != 66 && nr2 != 66))
        printf("B. %s\n", odpB);
    if (popodp[0] == 67 || (nr1 != 67 && nr2 != 67))
        printf("C. %s\n", odpC);
    if (popodp[0] == 68 || (nr1 != 68 && nr2 != 68))
        printf("D. %s\n", odpD);
    pasek();
    printf("Twoj wybor: ");
    znak = getchar();
    getchar(); // Gracz wprowadza swoj� odpowiedz
    ...
    if ((znak >= 97) && (znak <= 100)) // 97-100 to w kodzie ASCII a-D odejmujemy 32 aby uzyska� wielkie znaki A-D
    {
        znak = znak - 32;
    }
    printf("\n");
...
void telefon_do_przyjaciela(char pytanie[100], char odpA[100], char odpB[100], char odpC[100], char odpD[100], char popodp[2])
{
    ...
    sleep(3);
    czysc();
    int nr_losowy;          // od niej zale�y czy podpowiedz b�dzie trafna czy b��dna
    ...
    if (nr_losowy == 0)
    {
        int nr = rand() % 4 + 65;
        while (nr == popodp[0])
            nr = rand() % 4 + 65; // w przypadku gdzy nr_losowy=0 - losujemy odpowiedz r�n� od poprawnej w kodzie ASCII
        if (nr == 65)
            strcpy(tresc_odp, odpA);
        if (nr == 66)
            strcpy(tresc_odp, odpB);
        if (nr == 67)
            strcpy(tresc_odp, odpC);
        if (nr == 68)
            strcpy(tresc_odp, odpD);
    }
    else
    {
        if (popodp[0] == 65)
            strcpy(tresc_odp, odpA); // w przypadku gdzy nr_losowy=1 - za tresc_odp podstawiamy poprawn� odpowiedz
        if (popodp[0] == 66)
            strcpy(tresc_odp, odpB);
        if (popodp[0] == 67)
            strcpy(tresc_odp, odpC);
    ...
    sleep(3); // na ekranie wyswietla sie rozmowa z przyjacielem w kt�rej pada jego odpowiedz
