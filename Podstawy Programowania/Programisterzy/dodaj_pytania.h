...
void dodaj_pytania()
{
        ...
        for (int j = 0; j < 100; j++)
        {
            if (i == 0 || i == 4)
            {
                if (j == 99)
                    ...
                    printf("-"); // Nastepnie komunikaty (instrukcja), na koniec znów linia.
            }
            else
            {
                if (i == 1)
    ...
    fgets(pomocnicza, max_dlug, stdin); // Pobieramy pytanie o dlugosci max_dlug(100) znakow ze standardowego wejscia
    ...
    fgets(pomocnicza, max_dlug, stdin); // Analogicznie postepujemy pobierajac odpowiedzi A,B,C,D
    char odpA[strlen(pomocnicza)];
    przypisz(odpA, pomocnicza, strlen(pomocnicza) + 1);
    printf("Podaj odpowiedz B: ");
    fgets(pomocnicza, max_dlug, stdin);
    ...
    fgets(pomocnicza, max_dlug, stdin); // do 'pomocnicza' pobieramy znak poprawnej odpowiedzi
    while ((!((pomocnicza[0] >= 65) && (pomocnicza[0] <= 68))) && !((pomocnicza[0] >= 97) && (pomocnicza[0] <= 100)))
    {
        printf("Nieprawidlowa odpowiedz! Odpowiedz powinna byc literkami a lub b lub c lub d! Wprowadz poprawna odpowiedz: ");
        fgets(pomocnicza, max_dlug, stdin);
    }
